...
module ProductsHelper # rubocop:disable Style/Documentation
  ...
  def link_to_property_set(title, target)
    link_to(
      title,
      nil,
      class: 'property-set-link ml-3 mt-3 inline-flex items-center rounded-md border border-transparent bg-orange-600 px-2 py-1 text-sm font-small text-white shadow-sm hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2', # rubocop:disable Layout/LineLength
      data: { target: }
    )
  end
end
